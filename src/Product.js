...
export default function Product({name, background, decoration, basis, shadow, order, iconPath, titleTop, titleBottom, subtitleTop, subtitleBottom, buttonTextColor, buttonBorderColor}) {
  return (
    <section className={`group lg:min-h-[calc(100vh-72px)] pt-24 overflow-hidden relative h-full ${background} ${basis} ${shadow} ${order}`}>
      <div className="absolute inset-0 w-full h-full transition duration-[5000ms] bg-cover top-16 group-hover:scale-110" style={{backgroundImage: `url(${decoration})`}}>
      </div>
      ...
      <div className="relative mx-auto text-center lg:px-6 min-h-[calc(100vh-72px-12rem)]">
        ...
        <footer className="text-center lg:absolute lg:bottom-[5rem] lg:transform lg:left-1/2 lg:-translate-x-1/2 my-8 lg:my-0">
          <a href="#" className={`${buttonTextColor} ${buttonBorderColor} border bg-[#180B26] rounded text-lg px-6 py-3 inline-flex items-center gap-x-2 transition duration-500 group`}>
            Launch app
            <svg className="transition duration-500 transform group-hover:translate-x-1" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
